...
class MediaUITest extends WebTestBase {
  ...
  public function testMediaWithOnlyOneBundle() {
    ...
    $this->assertTitle($media->label() . ' | Drupal');

    // Test if the media list contains exactly 1 media bundle.
    $this->drupalGet('admin/content/media');
    $this->assertResponse(200);
    $this->assertText($media->label());

    // Make visible the "name" field on the entity form.
    $edit = [
      'fields[name][weight]' => -5,
      'fields[name][type]' => 'string_textfield',
    ];
  ...
  public function createMediaItem($media_bundle) {
    // Create media and retrieve new media item ID, then return all information.
    $this->drupalPostForm('media/add/' . $media_bundle['id'], [], t('Save and publish'));
    $media_id = \Drupal::entityQuery('media')->condition('bundle', $media_bundle['id'])->execute();
    $media_id = reset($media_id);
    /** @var \Drupal\media_entity\MediaInterface $media */
    $media = $this->container->get('entity_type.manager')
      ->getStorage('media')
      ->loadUnchanged($media_id);
    $edit['label'] = $media->label();
    $this->assertTitle($edit['label'] . ' | Drupal');
    $edit['id'] = $media_id;
  ...
  public function doTestMediaBundleFilter($first_media_bundle, $second_media_bundle) {
    ...
    $this->assertText($first_media_item['label']);
    $this->assertText($first_media_bundle['label']);
    $this->assertText($second_media_item['label']);
    ...
    $this->assertText($first_media_item['label']);
    $this->assertText($first_media_bundle['label']);
    $this->assertNoText($second_media_item['label']);

    $this->drupalGet('admin/content/media', ['query' => ['provider' => $second_media_bundle['id']]]);
    $this->assertResponse(200);
    $this->assertNoText($first_media_item['label']);
    $this->assertText($second_media_item['label']);
    $this->assertText($second_media_bundle['label']);
