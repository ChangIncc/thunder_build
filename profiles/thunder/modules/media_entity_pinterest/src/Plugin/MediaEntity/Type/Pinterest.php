...
class Pinterest extends MediaTypeBase {
  ...
  public static $validationRegexp = array(
    ...
    // Match BOARD_URL_RE.
    '@^\s*(https?://)?(\w+\.)?pinterest\.([a-zA-Z]+\.)?([a-zA-Z]+)/(?P<username>\w+)/(?P<slug>[\w\-_\~]+?)/?\s*$@i' => 'board',
    // Match USER_URL_RE.
    '@^\s*(https?://)?(\w+\.)?pinterest\.([a-zA-Z]+\.)?([a-zA-Z]+)/(?P<username>\w+)/?\s*$@i' => 'user',
  );
